...
int ptcRUN::colType(particle &part, readINPUT &input, cell *pin, readCX *CX){
	int col;
	double rn_col = RN.gnRN();
	double cdf_col;


	cdf_col = CX[matRUN - 1].total_scatter[EgroupRUN] / (CX[matRUN - 1].trans[EgroupRUN] - CX[matRUN - 1].fission[EgroupRUN]);
	if (rn_col < cdf_col){
		col = elas;
	}
	else{
...
void ptcRUN::simulateParticle(readINPUT &input, cell *pin, readCX *CX, CriData Data, CriticalBank &CriBank, double inweight, tally &tall, bool tflag){
	...
	//data_rollback = Data;
	ptc.MKptc(Data.x, Data.y, Data.z, Data.time, Data.En);
	ptc.weight = inweight;
	find_Group(ptc, input, pin, CX);
	index = (int)(ptc.r[0] / input.x_len_fi) + ((int)(ptc.r[1] / input.y_len_fi))*input.x_num_fi;
	do{

		//		roll_back(ptc,input,pin,Data,CX,inweight);
		...
		if (dtc_s < dts_s){// collision
			...
			ptc.weight *= CX[matRUN - 1].total_scatter[EgroupRUN] / CX[matRUN - 1].trans[EgroupRUN];
			//russian rulette
			if (ptc.weight < 0.2) {
				rn = RN.gnRN();
				if (ptc.weight / 0.5 > rn)
					ptc.weight = 0.5;
				else
					break;//sc=End;
			}
			...
			ptc.MKptc(ptc.r[0], ptc.r[1], ptc.r[2], ptc.t, ptc.En);
		}
		else{// next cell

			flight(dts_s + delta, ptc);
			if (tflag)updateTally(input, pin, CX, tall, dts_s, ptc.weight);
			esti_T(ptc, input, pin, CX, tall, ptc.weight, dts_s);
...
/*void ptcRUN::roll_back(particle &part, readINPUT &input, cell *pin, CriData &Data, readCX *CX, double inweight){
	int i;
	double x_in, y_in;
	x_in = part.r[0] - (int)(part.r[0] / input.x_len_fi)*input.x_len_fi;
	y_in = part.r[1] - (int)(part.r[1] / input.y_len_fi)*input.y_len_fi;

	// x left right, y bottom top, z bottom top
	outside_check[0] = ((part.r[0] <= pin[0].xl));
	outside_check[1] = ((part.r[0] >= pin[input.x_num_fi*input.y_num_fi - 1].xh));
	outside_check[2] = ((part.r[1] <= pin[0].yl));
	outside_check[3] = ((part.r[1] >= pin[input.x_num_fi*input.y_num_fi - 1].yh));
	outside_check[4] = ((part.r[2] <= pin[0].zl));
	outside_check[5] = ((part.r[2] >= pin[0].z_ref));
	outside_check[6] = 0;
	for (i = 0; i < 6; i++){
		outside_check[6] += outside_check[i];
	}
	...
	if (outside_check[6] != 0){
		cout << "roll_back at " << ptc.r[0] << "  " << ptc.r[1] << "  " << ptc.r[2] << "  " << endl;
		Data = data_rollback;
		ptc.MKptc(Data.x, Data.y, Data.z, Data.time, Data.En);
		ptc.weight = inweight;
		find_Group(ptc, input, pin, CX);
		index = (int)(ptc.r[0] / input.x_len_fi) + (int)(ptc.r[1] / input.y_len_fi)*input.y_num_fi;
